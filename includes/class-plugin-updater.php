 ...
 *
 * Namespace: JPKComDisableXmlrpcGitUpdate
 * PHP Version: 8.3+
 ...
 *
 * @package JPKComDisableXmlrpcGitUpdate
 */
    ...
    public function verify_download_checksum( $reply, string $package, \WP_Upgrader $upgrader ) {
        ...
        if ( ! $remote ) {
            return new \WP_Error(
                ...
                __( 'Security verification failed: the update manifest could not be loaded, so the package cannot be verified.', 'jpkcom-disable-xmlrpc' )
            );
        }
        ...
        if ( empty( $remote->checksum_sha256 ) ) {
            return new \WP_Error(
                'checksum_missing',
                __( 'Security verification failed: the update manifest contains no SHA-256 checksum, so the package cannot be verified.', 'jpkcom-disable-xmlrpc' )
            );
        }
        ...
        if ( is_wp_error( $temp_file ) ) {
            return new \WP_Error(
                'download_failed',
                sprintf(
                    __( 'Download failed: %s', 'jpkcom-disable-xmlrpc' ),
        ...
        if ( ! is_string( $calculated_hash ) || ! hash_equals( $expected_hash, $calculated_hash ) ) {
            ...
            $error_msg = sprintf(
                ...
                __( 'Security verification failed: Plugin checksum mismatch. Expected: %1$s, Got: %2$s', 'jpkcom-disable-xmlrpc' ),
                $expected_hash,
